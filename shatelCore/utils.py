...
def add_watermark(logo_full_path: str, filename, position, outputname: str, scale: int = 15, padding: int = 5):
    """
...
@shared_task(store_result=True)
def compress_image(image_full_path, quality: int = 50):
    """
    compress image size
    Args:
    """
    ...
    image.save(image_full_path, optimize=True, quality=quality)
    return back(True)
